...
# Timestamp: 2022/02/03 16:42:54 UTC
# 
...
RUN bash -c 'source activate mvpa && cd /home/neuro               && pip install -U pip              && pip install -U numpy              && git clone git://github.com/PyMVPA/PyMVPA.git              && cd PyMVPA              && make 3rd              && python setup.py build_ext              && python setup.py install              && cd ..              && rm -rf PyMVPA              && source deactivate mvpa'
...
RUN bash -c 'source activate neuro && pip install -U pip && pip install -U numpy nbval'
...
RUN echo '{ \
    ...
    \n      "cmd", \
    \n      [ \
    \n        "jupyter-notebook --port=8888 --no-browser --ip=0.0.0.0 --NotebookApp.token=fmriflows" \
    \n      ] \
    \n    ] \
    \n  ] \
